...
window.Werkmap=(()=>{
 ...
 async function backup(){
  if(busy)return;
  if(!root){message('Kies eerst een werkmap en bewaar daarin je werk.');return;}
  if(!confirm('Heb je je wijzigingen in alle tools bewaard? De back-up bevat alle bestanden in je gekozen werkmap, ook eventuele andere bestanden. Onbewaarde invoer en bestanden buiten deze map gaan niet mee. Bewaar tijdens het maken van de back-up niet in andere vensters. Doorgaan?'))return;
  busy=true;$('backup').disabled=true;
  try{
   await permission(root);await checkRoot();message('Back-up maken van '+root.name+'…');
   if(!window.GereedschapskistBackup)await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=new URL('werkmap-backup.js',document.querySelector('script[src$="werkmap.js"]').src).href;s.onload=resolve;s.onerror=()=>{s.remove();reject(Error('De back-upfunctie kon niet worden geladen.'));};document.head.append(s);});
   const {blob,count}=await GereedschapskistBackup(root);const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='gereedschapskist-back-up-'+new Date().toISOString().replace(/[:.]/g,'-')+'.zip';a.click();setTimeout(()=>URL.revokeObjectURL(url),60000);
   message('Download gestart: '+count+' bestanden. Controleer of je ZIP is opgeslagen. Herstellen: pak de ZIP uit en kies de uitgepakte map als werkmap. Je bestaande map blijft zo behouden.');
  }catch(e){message('Back-up niet gemaakt: '+e.message);}finally{busy=false;$('backup').disabled=false;}
 }
 function render(){if(!box)return;if(!tool){for(const link of document.querySelectorAll('.grid a')){if(!link.dataset.originalHref)link.dataset.originalHref=link.getAttribute('href');const url=new URL(link.dataset.originalHref,location.href);if(root)url.searchParams.set('werkruimte','eigen');link.href=root?url.href:link.dataset.originalHref;}}$('name').textContent=example?'Voorbeelden blijven buiten je eigen werkmap':root?'Werkmap: '+root.name+(tool?' / '+catalog[tool][0]:''):'Mijn werkmap';$('choose').hidden=!supported||example;$('choose').textContent=root?'Andere werkmap kiezen':'Kies werkmap';$('backup').hidden=!!example;$('forget').hidden=!root;$('open').hidden=!root||!adapter;$('copy').hidden=!root||!adapter;$('hint').textContent=example?'Je kunt voorbeelden downloaden om te oefenen.':!supported?'Deze browser biedt geen maptoegang. Bestand openen en downloaden blijven beschikbaar.':root?'Bewaar bestand schrijft naar deze map. Open een bestaand werkmapbestand eerst via Open uit werkmap. CSV, bonnen en overdrachten komen in Exports. PDF’s kies je zelf in het afdrukvenster.':'Kies één map voor je werk. De negen submappen worden aangemaakt. Zonder werkmap blijft Bewaar bestand een download.';const loose=document.querySelector('.loose-documents span');if(loose)loose.textContent=root?'Bewaar bestand schrijft naar Schrijven in je werkmap. Download kopie maakt een losse download.':'Bewaar bestand maakt een download. Opslaan in map werkt alleen na Map openen.';}
