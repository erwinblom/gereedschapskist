...
window.GereedschapskistExamples = function(tool) {
 ...
 const projectId='voorbeeld-plan-1',projectName='Buurtwerkplaats De Proeftuin opzetten';
 ...
 const contacts={format:'contacten',version:1,contacts:[
  {id:'voorbeeld-sam',name:'Sam Voorbeeld',organization:'Buurtwerkplaats De Proeftuin',role:'Projectcoördinator',address:'Oefenplein 2\n1234 AB Voorbeeldstad',email:'sam@example.com',phone:'',tags:['buurtwerkplaats','opdrachtgever'],notes:'Fictief contact. Coördineert de eerste reparatiemiddag.',nextDate:day(1),nextAction:'Voorstel voor de workshop bespreken',conversations:[{id:'voorbeeld-gesprek',date:day(-2),text:'Oefennotitie: we willen een toegankelijke werkplaats waar buurtbewoners samen repareren en kennis delen.'}]},
 ...
 switch(tool){
  ...
  case 'Ping': return {format:'ping-local',version:2,sequences:{},business:{...company,iban:''},invoices:[{id:'voorbeeld-factuur',sourceQuoteId:quote.id,sourceContactId:'voorbeeld-sam',state:'draft',title:quote.title,date,deliveryDate:date,due:day(14),customer:quote.customer,contactPerson:quote.contactPerson,address:quote.address,email:quote.email,note:'OEFENFACTUUR — niet versturen. Deze factuur hoort bij een fictieve workshop.',lines:[{description:'Voorbereiding (uren)',quantity:2,cents:7500,vat:21},{description:'Reparatieworkshop begeleiden',quantity:1,cents:45000,vat:21}]}]};
  case 'Projectbord': return {format:'projectbord',version:1,name:'Buurtwerkplaats De Proeftuin',tasks:[
   {id:'voorbeeld-taak-1',projectId,project:projectName,contactId:'voorbeeld-sam',title:'Een ruimte voor de proefmiddag vinden',notes:'Sam bekijkt welke plek toegankelijk is en voldoende werktafels heeft.',state:'doing',priority:'high',due:day(3)},
   {id:'voorbeeld-taak-2',projectId,project:projectName,contactId:'voorbeeld-noor',title:'Gereedschap verzamelen',notes:'Noor vraagt vrijwilligers welk gereedschap zij kunnen meenemen.',state:'todo',priority:'normal',due:day(5)},
   {id:'voorbeeld-taak-3',projectId,project:projectName,contactId:'voorbeeld-sam',title:'Bewoners uitnodigen',notes:'Maak een korte uitnodiging met tijd, plek en wat mensen kunnen meenemen.',state:'todo',priority:'normal',due:day(7)},
   {id:'voorbeeld-taak-4',projectId,project:projectName,contactId:'voorbeeld-sam',title:'Idee met bewoners bespreken',notes:'Fictief voorbeeld: de eerste reacties zijn verzameld.',state:'done',priority:'normal',due:day(-2)}]};
  case 'Bronnenkast': return {...structuredClone(sources),name:'Inspiratie voor Buurtwerkplaats De Proeftuin',items:structuredClone(sources.items.filter(i=>['start-02','start-04','start-07'].includes(i.id))).map(i=>({...i,notes:'Voor ons voorbeeldproject: '+({'start-02':'hoe organiseren we een toegankelijke reparatiemiddag?','start-04':'welke kennis kunnen vrijwilligers delen over hergebruik?','start-07':'hoe laten we beginners zelf iets maken?'}[i.id]),favorite:i.id==='start-02'}))};
  case 'Uren': return {format:'uren',version:1,entries:[{id:'voorbeeld-uren-1',date,client:'Buurtwerkplaats De Proeftuin',project:projectName,projectId,contactId:'voorbeeld-sam',description:'Workshop voorbereiden',minutes:120,rateCents:7500,vat:21,billable:true},{id:'voorbeeld-uren-2',date,client:'Buurtwerkplaats De Proeftuin',project:projectName,projectId,contactId:'voorbeeld-sam',description:'Workshop begeleiden',minutes:180,rateCents:15000,vat:21,billable:true},{id:'voorbeeld-uren-3',date,client:'Eigen organisatie',project:projectName,projectId,description:'Ideeën en bronnen verzamelen',minutes:45,billable:false}]};
