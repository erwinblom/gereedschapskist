...
(()=>{
 ...
 document.querySelector('.heading').after(controls,message);controls.append(receive,help);
 ...
 receive.onclick=async()=>{
  if(GereedschapskistMode.example){message.textContent='Kies eerst Begin met mijn eigen werk. Persoonlijke links komen niet tussen de voorbeelden.';return;}
  ...
  try{
   ...
   message.textContent=`${fresh.length} nieuwe links ontvangen. Je vindt ze in de categorie Inbox. Bewaar je collectie met Bewaar alles of Bewaar bestand.`+(remaining.pending?` Nog ${remaining.pending} links te ontvangen; klik opnieuw op Ontvang links.`:'');
