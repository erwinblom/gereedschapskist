        ...
        function loadConverterFiles() {
            try {
                const saved = localStorage.getItem('gereedschapskist:converterFiles');
                if (saved) {
        ...
        function saveConverterFiles() {
            try {
                ...
                localStorage.setItem('gereedschapskist:converterFiles', JSON.stringify(files));
            } catch (err) {
        ...
        // Theme management
        function initTheme() {
            const savedTheme = localStorage.getItem('gereedschapskist:theme');
            if (savedTheme) {
        ...
        function toggleTheme() {
            ...
            localStorage.setItem('gereedschapskist:theme', newTheme);
        }
        ...
        if (window.matchMedia) {
            window.matchMedia('(prefers-color-scheme: light)').addEventListener('change', (e) => {
                if (localStorage.getItem('gereedschapskist:theme') === 'system') {
                    if (e.matches) {
                        document.documentElement.setAttribute('data-theme', 'light');
                    } else {
                        document.documentElement.removeAttribute('data-theme');
        ...
        const DB_NAME = 'gereedschapskist:MarkdownWerkbankLocalV2';
        ...
        async function clearIndexedDB() {
            ...
            try {
                ...
                localStorage.removeItem('gereedschapskist:converterFiles');
        ...
        async function loadAllFileContents() {
            ...
            try { for (const path of JSON.parse(localStorage.getItem('gereedschapskist:mw-folders-'+selectedProject)||'[]')) expandedFolders.add(path); } catch {}
            renderFileList();
            restoreLastOpenFile();
        }
        ...
        function restoreLastOpenFile() {
            ...
            try {
                const savedPath = localStorage.getItem('gereedschapskist:mw-document-'+selectedProject);
        ...
        async function selectFile(index) {
            ...
            try {
                localStorage.setItem('gereedschapskist:lastOpenFile', file.relativePath);
                localStorage.setItem('gereedschapskist:mw-document-'+selectedProject, file.relativePath);
            } catch (e) {}
        ...
        function openDeleteFileDialog() {
            ...
            dialog.querySelector('form').onsubmit = async event => {
                ...
                try { localStorage.removeItem('gereedschapskist:lastOpenFile'); } catch (_) {}
        ...
        (function() {
            ...
            document.addEventListener('mouseup', () => {
                if (isResizing) {
                    ...
                    localStorage.setItem('gereedschapskist:sidebarWidth', width);
                }
            });

            // Restore saved width
            const savedWidth = localStorage.getItem('gereedschapskist:sidebarWidth');
            if (savedWidth) {
