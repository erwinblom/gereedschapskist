...
try { selectedProject=localStorage.getItem('gereedschapskist:mw-project')||'all'; } catch {}
...
function rememberProject(){try{localStorage.setItem('gereedschapskist:mw-folders-'+selectedProject,JSON.stringify([...expandedFolders]));if(activeFile&&projectIncludes(activeFile.relativePath))localStorage.setItem('gereedschapskist:mw-document-'+selectedProject,activeFile.relativePath);}catch{}}
...
function defaultStartPath(){let saved;try{saved=localStorage.getItem('gereedschapskist:mw-start-'+selectedProject);}catch{}return projectStartPath([...folderHandlesByPath.keys()],selectedProject,saved);}
function updateProjectControls(){
 ...
 try{document.getElementById('saveStartFolder').checked=localStorage.getItem('gereedschapskist:mw-start-'+selectedProject)===target.value;}catch{}
...
async function switchProject(name){
 ...
 rememberProject();selectedProject=name;try{localStorage.setItem('gereedschapskist:mw-project',name);}catch{}
 ...
 try{for(const p of JSON.parse(localStorage.getItem('gereedschapskist:mw-folders-'+name)||'[]'))expandedFolders.add(p);}catch{}
...
document.getElementById('newItemTarget').onchange=()=>{const target=document.getElementById('newItemTarget');try{document.getElementById('saveStartFolder').checked=localStorage.getItem('gereedschapskist:mw-start-'+selectedProject)===target.value;}catch{document.getElementById('saveStartFolder').checked=false;}};
document.getElementById('saveStartFolder').onchange=event=>{const path=document.getElementById('newItemTarget').value;if(selectedProject==='all'||!path)return;try{if(event.target.checked)localStorage.setItem('gereedschapskist:mw-start-'+selectedProject,path);else localStorage.removeItem('gereedschapskist:mw-start-'+selectedProject);}catch{event.target.checked=false;showNotification('De bestemming kon niet worden onthouden in deze browser.','error');}};
