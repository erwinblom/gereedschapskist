...
try { selectedProject=GereedschapskistMode.storage.getItem('gereedschapskist:mw-project')||'all'; } catch {}
...
function rememberProject(){try{GereedschapskistMode.storage.setItem('gereedschapskist:mw-folders-'+selectedProject,JSON.stringify([...expandedFolders]));if(activeFile&&projectIncludes(activeFile.relativePath))GereedschapskistMode.storage.setItem('gereedschapskist:mw-document-'+selectedProject,activeFile.relativePath);}catch{}}
...
function updateProjectControls(){
 ...
 try{document.getElementById('saveStartFolder').checked=GereedschapskistMode.storage.getItem('gereedschapskist:mw-start-'+selectedProject)===target.value;}catch{}
...
async function switchProject(name){
 ...
 rememberProject();selectedProject=name;try{GereedschapskistMode.storage.setItem('gereedschapskist:mw-project',name);}catch{}
 ...
 try{for(const p of JSON.parse(GereedschapskistMode.storage.getItem('gereedschapskist:mw-folders-'+name)||'[]'))expandedFolders.add(p);}catch{}
...
document.getElementById('newItemTarget').onchange=()=>{const target=document.getElementById('newItemTarget');try{document.getElementById('saveStartFolder').checked=GereedschapskistMode.storage.getItem('gereedschapskist:mw-start-'+selectedProject)===target.value;}catch{document.getElementById('saveStartFolder').checked=false;}};
document.getElementById('saveStartFolder').onchange=event=>{const path=document.getElementById('newItemTarget').value;if(selectedProject==='all'||!path)return;try{if(event.target.checked)GereedschapskistMode.storage.setItem('gereedschapskist:mw-start-'+selectedProject,path);else GereedschapskistMode.storage.removeItem('gereedschapskist:mw-start-'+selectedProject);}catch{event.target.checked=false;showNotification('De bestemming kon niet worden onthouden in deze browser.','error');}};
