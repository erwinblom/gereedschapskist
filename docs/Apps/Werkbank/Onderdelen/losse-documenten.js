...
(()=>{
 ...
 bar.innerHTML='<button id="loose-open">Document openen</button><button id="loose-new">Nieuw document</button><button id="loose-save" disabled>Bewaar document</button><span>Open een Markdown-bestand of kies een projectmap. Bewaar zelf een bestand op je computer.</span><input type="file" id="loose-input" accept=".md,.markdown,.txt,text/markdown,text/plain" hidden>';
 ...
 async function addLoose(name,content){
  ...
  showNotification('Document geopend. Gebruik Bewaar document om een eigen bestand te bewaren.','success');
 }
